...
data_folder = os.path.join(project_root, 'data', 'Timeslot by week')
...
output_dir = os.path.join(project_root, 'data', 'Timeslot_Yearly_Analysis')
...
# Generate yearly average daily timeslot patterns
print("\n=== Generating yearly average daily timeslot patterns ===")

def plot_yearly_average_daily_pattern(year_data, category, direction):
    """Plot yearly average daily timeslot pattern"""
    # For R&P, exclude data after September (months 10-12) due to data quality issues
    if category == 'R&P':
        filtered_data = year_data[year_data['Date_parsed'].dt.month <= 9]
        print(f"  R&P: Using only Jan-Sep data (excluding Oct-Dec due to data issues)")
    else:
        filtered_data = year_data
    
    # Extract data for the entire year (or filtered period)
    timeslot_data = extract_timeslot_data(filtered_data, direction, category)
    
    # Calculate number of days in the data
    num_days = len(filtered_data[filtered_data.iloc[:, 3] == category]['Date_parsed'].unique())
    ...
    avg_booking_taken = timeslot_data['Booking Taken'] / num_days
    avg_available = timeslot_data['Available Capacity'] / num_days
    
    # Create figure with two subplots
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(14, 10))
    
    # Plot 1: Stacked bar chart with utilization
    x = timeslot_data['Time Slot']
    
    ax1.bar(x, avg_booking_taken, label='Booking Taken (Avg/Day)', 
            color='steelblue', alpha=0.8)
    ax1.bar(x, avg_available, bottom=avg_booking_taken, 
            label='Available Capacity (Avg/Day)', color='lightcoral', alpha=0.8)
    
    # Add utilization rate labels
    total_capacity = avg_booking_taken + avg_available
    for i, (bt, total) in enumerate(zip(avg_booking_taken, total_capacity)):
        if total > 0:
            util_rate = bt / total
            if util_rate > 0.05:
                ax1.text(i, total/2, f'{util_rate:.0%}', ha='center', va='center', 
                        fontsize=8, fontweight='bold', color='white')
    
    ax1.set_xlabel('Time Slot (Hour)', fontsize=12)
    ax1.set_ylabel('Average Daily Capacity', fontsize=12)
    period_text = '2025 Jan-Sep' if category == 'R&P' else '2025 Full Year'
    ax1.set_title(f'{category} - {direction.capitalize()} - {period_text} Average Daily Slot Utilization', 
                  fontsize=14, fontweight='bold')
    ...
    # Plot 2: Utilization rate line chart
    utilization_rate = timeslot_data['Utilization Rate'].fillna(0).values  # Convert to numpy array and fill NaN
    
    ax2.plot(x, utilization_rate, marker='o', linewidth=2, 
             markersize=6, color='darkgreen', label='Utilization Rate')
    ax2.fill_between(x, 0, utilization_rate, alpha=0.3, color='lightgreen')
    
    ax2.set_xlabel('Time Slot (Hour)', fontsize=12)
    ax2.set_ylabel('Utilization Rate', fontsize=12)
    period_text = '2025 Jan-Sep' if category == 'R&P' else '2025 Full Year'
    ax2.set_title(f'{category} - {direction.capitalize()} - {period_text} Average Hourly Utilization Rate', 
                  fontsize=14, fontweight='bold')
    ax2.set_xticks(range(24))
    ax2.set_xticklabels(range(24))
    ax2.set_ylim(0, 1)
    ax2.axhline(y=0.5, color='orange', linestyle='--', linewidth=1, alpha=0.7, label='50% Threshold')
    ax2.axhline(y=0.8, color='red', linestyle='--', linewidth=1, alpha=0.7, label='80% Threshold')
    ax2.legend(loc='upper left')
    ax2.grid(True, alpha=0.3)
    
    # Add statistics text
    avg_util = np.nanmean(utilization_rate)
    max_util = np.nanmax(utilization_rate)
    max_hour = int(np.nanargmax(utilization_rate))
    min_util = np.nanmin(utilization_rate)
    min_hour = int(np.nanargmin(utilization_rate))
    
    stats_text = f'Avg Utilization: {avg_util:.1%}\n'
    stats_text += f'Peak Utilization: {max_util:.1%} (Hour {max_hour})\n'
    stats_text += f'Min Utilization: {min_util:.1%} (Hour {min_hour})\n'
    stats_text += f'Total Days: {num_days}'
    ...
    plt.tight_layout()
    filename = f'{category}_{direction}_Yearly_Average_Daily_Pattern_2025.png'
    filepath = os.path.join(output_dir, filename)
    plt.savefig(filepath, dpi=300, bbox_inches='tight')
    plt.close()
    print(f"  Generated {filename}")

# Generate yearly average daily patterns for all categories
plot_yearly_average_daily_pattern(year_data, 'FG', 'inbound')
plot_yearly_average_daily_pattern(year_data, 'FG', 'outbound')
plot_yearly_average_daily_pattern(year_data, 'R&P', 'inbound')
plot_yearly_average_daily_pattern(year_data, 'R&P', 'outbound')
