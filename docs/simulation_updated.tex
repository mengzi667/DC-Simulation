...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        % UPDATED: Changed from 12 months to 8 months data range
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \node[decision, right=1cm of loading] (check) {Timeslot\\Check};
        ...
        \node[constraint, below=1.4cm of queue] (timeslot_const) {Hourly\\Timeslot Limit};
        
        \node[constraint, below=1.4cm of check] (capacity) {Timeslot\\Availability};
        ...
        \draw[arrow] (check) -- node[above, font=\small] {Success} (depart);
        
        % ========== Delayed Path ==========
        \node[entity, above=2.3cm of check, fill=yellow!20] (delayed) {Delayed\\(Missed Slot)};
        \draw[arrow, orange] (check) -- node[right, font=\footnotesize] {Missed} (delayed);
...
All numerical inputs are provided via an external configuration file generated by a dedicated data-preparation pipeline. The configuration includes (i) hourly arrival-rate profiles for inbound and outbound, (ii) hourly dock/timeslot capacity profiles, (iii) pallet-load distributions by category, (iv) labor (FTE) configuration and productivity parameters, and (v) unloading-efficiency parameters. The configuration file also records data sources and an extraction date to ensure traceability.

\subsection*{Data Sources and Temporal Scope}
\label{subsec:data_provenance}

% UPDATED: Specified 8-month data range (previously implied 12 months)
...
Inbound arrivals are governed by hourly arrival-rate profiles for FG and R\&P derived from historical shipment data. In each simulated hour, the number of inbound arrivals is sampled from a Poisson distribution whose rate parameter equals the configured arrival rate for that category and hour. Within the hour, each sampled arrival is further spread by an additional arrival delay drawn from an exponential distribution with a mean of 0.25 hours (15 minutes), to avoid simultaneous arrivals at the top of the hour.

\subsection{Outbound Order Generation and Scheduling}
\label{subsec:outbound_arrival_rates}

% UPDATED: Changed from Poisson arrival to pre-generated order-driven mechanism
...
\begin{table}[H]
    ...
    \end{tabular}
    \\[0.2cm]
    % UPDATED: Modified table note to reflect order-driven outbound mechanism
...
% UPDATED: Specified data source as months 1-8 for pallet distributions
...
Dock (timeslot) capacities are specified as hourly integer capacities for loading and reception, separately for FG and R\&P. These capacities are extracted from weekly timeslot data by aggregating booked and available capacities and averaging per hour across observed days. The extraction logic is implemented in the data-preparation script and considers only hours where actual booking activity occurred.

\subsection{Unloading Efficiency and Unloading Time Generation}
\label{subsec:unloading_time}

Inbound unloading time is generated stochastically. The simulation samples an unloading efficiency from a normal distribution, separately parameterized for FG and R\&P, and then computes unloading time as ``pallets divided by efficiency''. To prevent non-physical values, sampled efficiencies are truncated below by a positive lower bound prior to computing unloading time.
...
% UPDATED: Added detailed region-based service level rules (G2/ROW)
...
The FG outbound flow models the release and execution of outbound shipments under dock-capacity constraints and timeslot requirements.

\paragraph{Step 1: Order activation at creation time.}
Orders are pre-generated with assigned \texttt{creation\_time}, \texttt{timeslot\_time}, and \texttt{region} attributes. For FG outbound, the region (G2\_same\_day, G2\_next\_day, or ROW\_next\_day) determines the service-level requirement. At simulation time equal to \texttt{creation\_time}, the order is activated and enters the processing queue.

\paragraph{Step 2: Internal processing stage (labor-constrained).}
After activation, the order enters an internal processing stage. The simulation records the start and end timestamps of processing. Processing duration is determined by the relationship between pallet volume and the effective internal processing capacity available for FG outbound (labor/FTE-based).

\paragraph{Step 3: Waiting for pre-allocated timeslot.}
Upon completion of internal processing, the order waits until its pre-allocated \texttt{timeslot\_time} arrives. The simulation tracks whether processing completes in time for the allocated slot.

\paragraph{Step 4: Loading dock acquisition (timeslot-constrained).}
At the allocated timeslot hour, the order attempts to acquire an available loading dock position for FG. If capacity is exhausted, the order experiences a timeslot miss. When a dock position is acquired, the simulation records the dock service start time and logs any waiting time.

\paragraph{Step 5: Loading execution and completion.}
The model then executes the loading activity as a service operation occupying the acquired dock position. The service end time is recorded upon completion.

\paragraph{Step 6: Performance logging.}
The simulation logs whether the order loaded at its allocated timeslot (on-time) or experienced a delay. For FG outbound, the order's region is also recorded to enable regional performance analysis. The outbound operation is logged for throughput accounting.

\subsection{R\&P Outbound Flow}
\label{subsec:rp_outbound_logic}

The R\&P outbound flow follows the same structure as FG outbound: orders are pre-generated with creation times and timeslot allocations, processed through FTE-constrained preparation, and loaded at their allocated slots subject to dock capacity constraints.

\paragraph{Step 1: Order activation.}
R\&P outbound orders are triggered at their \texttt{creation\_time} and assigned a pallet load.

\paragraph{Step 2: Internal processing stage (labor-constrained).}
Each R\&P outbound order passes through an internal processing stage constrained by the effective R\&P outbound processing capacity. Processing start and end timestamps are recorded.

\paragraph{Step 3: Waiting for timeslot and loading.}
After processing, the order waits for its pre-allocated timeslot, acquires dock capacity when available, and completes loading. Performance metrics are logged.
...
The model logs a ``midnight backlog'' each day to track pending orders and workload accumulation.

\paragraph{Volume monitoring.}
The model keeps tracking different types of orders' volume as well as the timeslot compliance states and records performance metrics.
...
\subsection{Outbound Timeslot Compliance (Overall and by Region)}
\label{subsec:kpi_sla}

% UPDATED: Added regional breakdown (G2 vs ROW) to compliance metrics
\paragraph{What it measures.}
...
\paragraph{How it is computed.}
For each outbound order, the model records whether loading occurred at the hour matching the order's \texttt{timeslot\_time}. The overall compliance rate is:
\[
\text{Timeslot compliance rate} = \frac{\#\text{orders loaded at allocated slot}}{\#\text{total orders processed}}.
\]

For FG outbound, regional compliance rates are computed separately for G2 and ROW by filtering orders based on their \texttt{region} attribute prefix:
\[
\text{G2 compliance rate} = \frac{\#\text{G2 orders on-time}}{\#\text{total G2 orders}},\quad
\text{ROW compliance rate} = \frac{\#\text{ROW orders on-time}}{\#\text{total ROW orders}}.
\]

\paragraph{How it is used.}
Timeslot compliance is a core service-risk KPI. In scenario analysis, any opening-time reduction that materially reduces compliance indicates a service degradation due to processing delays or capacity constraints preventing timely loading. The regional decomposition reveals whether service impacts are uniform or disproportionately affect specific customer segments (G2 vs ROW).
...
\paragraph{How it is computed.}
The simulation logs inbound and outbound operations with pallet quantities. The summary aggregates total pallets and counts of operations by category and direction:
\[
...
\paragraph{How it is used.}
Throughput supports both performance interpretation and sanity checking. If order generation is consistent across scenarios, significant reductions in realized throughput indicate that the system fails to clear arriving workload due to capacity constraints.
...
\begin{itemize}
    ...
    \item Order triggering: verified that outbound orders activate at their specified \texttt{creation\_time}
    \item Timeslot allocation: verified that orders are assigned to their pre-allocated timeslot hours
...
% UPDATED: Explicitly stated 8-month validation data range
